...
const registry: Record<string, FluxIconTuple> = {};
...
export function FluxIcon({ariaLabel, className, color, iconStyle, name, onClick, size}: FluxIconProps) {
    ...
    if (iconConfig.renderMode === 'svg' && definition) {
        ...
        return (
            <svg
                className={clsx(iconStyles.fontAwesomeIcon, color && colorClasses[color], className)}
                viewBox={`0 0 ${width} ${height}`}
                style={{fontSize, scale: width / 512 > 1 ? width / 512 : undefined}}
                focusable="false"
    ...
    if (iconConfig.renderMode === 'font' && name) {
        return (
            <i
                className={clsx(iconStyles.iconFont, fontStyleClasses[resolvedStyle], color && colorClasses[color], className)}
                style={{fontSize}}
                role={ariaLabel ? 'img' : undefined}
                aria-hidden={ariaLabel ? undefined : true}
                aria-label={ariaLabel}
            ...
            >
                {resolvedStyle === 'duotone' ? <><span className={iconStyles.iconFontSecondary}>{name}##</span><span className={iconStyles.iconFontPrimary}>{name}#</span></> : name}
            </i>
        );
    }
