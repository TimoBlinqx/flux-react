...
export const FluxFormInput = forwardRef<HTMLInputElement, FluxFormInputProps>(function FluxFormInput({className, disabled, error, iconLeading, iconTrailing, isCondensed, isLoading, isReadonly, isSecondary, onChange, onValueChange, readOnly, type = 'text', ...props}, ref) {
    ...
    return <div className={clsx(disabled ? formStyles.formInputDisabled : formStyles.formInputEnabled, isCondensed && formStyles.isCondensed, isSecondary && formStyles.isSecondary, (error || field?.error) && formStyles.isInvalid, className)} aria-disabled={disabled || undefined}>
        <input
            {...props}
            ref={ref}
        ...
        {type === 'password' ? <button type="button" className={formStyles.formInputIconPasswordToggle} aria-label="Toggle password visibility" aria-pressed={passwordVisible} disabled={disabled} onClick={() => setPasswordVisible(value => !value)}><FluxIcon name={passwordVisible ? 'eye-slash' : 'eye'} size={18} /></button> : iconTrailing ? <FluxIcon className={formStyles.formInputIconTrailing} name={iconTrailing} size={18} /> : null}
...
export const FluxFormTextArea = forwardRef<HTMLTextAreaElement, FluxFormTextAreaProps>(function FluxFormTextArea({className, disabled, error, isCondensed, isReadonly, isSecondary, onChange, onValueChange, readOnly, rows = 3, style, ...props}, ref) {
    ...
    return <textarea
        {...props}
        ref={ref}
        className={clsx(disabled ? formStyles.formTextAreaDisabled : formStyles.formTextAreaEnabled, isCondensed && formStyles.isCondensed, isSecondary && formStyles.isSecondary, (error || field?.error) && formStyles.isInvalid, className)}
        id={props.id ?? field?.id}
        disabled={disabled}
        readOnly={isReadonly ?? readOnly}
        rows={rows}
...
export const FluxFormCheckbox = forwardRef<HTMLInputElement, FluxFormCheckboxProps>(function FluxFormCheckbox({checked = false, className, disabled, error, isReadonly, label, onCheckedChange, subLabel, ...props}, forwardedRef) {
    ...
    return <label className={clsx(formStyles.formCheckbox, disabled && formStyles.isDisabled, isReadonly && formStyles.isReadonly, (error || field?.error) && formStyles.isInvalid, className)}>
        <input
            {...props}
            ref={setRef}
            type="checkbox"
            ...
            onChange={event => {if (!isReadonly && !disabled) onCheckedChange?.(event.target.checked);}}
        />
...
export function FluxToggle({checked = false, className, disabled, error, iconOff, iconOn, isReadonly, onCheckedChange, ...props}: FluxToggleProps) {
    const field = useContext(FieldContext);
    return <label className={clsx(formStyles.formToggle, checked && formStyles.isChecked, disabled && formStyles.isDisabled, isReadonly && formStyles.isReadonly, (error || field?.error) && formStyles.isInvalid, className)}>
        ...
        <input
            ...
            aria-invalid={Boolean(error || field?.error) || undefined}
            onClick={event => {if (isReadonly) event.preventDefault(); props.onClick?.(event);}}
            onChange={event => {if (!isReadonly) onCheckedChange?.(event.target.checked);}}
        />
    </label>;
}
